...
export default function ShareButton() {
    ...
    return (
        <div ref={shareRef} className="share-article absolute top-0 right-0 sm:-right-5 lg:-right-12 gap-3 flex flex-col">
            <h6 className="text-xs leading-[110%]">Share</h6>
            
            {/* Share trigger button - always visible */}
            <div 
                onClick={handleExpand} 
                aria-label='share-button' 
                className="rounded-full grid lg:hidden cursor-pointer place-items-center shadow-sm size-[34px] xl:size-9 3xl:size-10 bg-white"
            >
            ...
            <div className={`flex flex-col gap-3 ${expand ? 'flex' : 'hidden lg:flex'}`}>
                <div className="rounded-full grid cursor-pointer place-items-center shadow-sm size-[34px] xl:size-9 3xl:size-10 bg-white hover:bg-gray-100 transition-colors">
