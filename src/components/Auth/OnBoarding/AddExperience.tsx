...
export default function AddExperience() {
      ...
      try {
        ...
        Object.entries(payload).forEach(([key, value]) => {
          ...
          if (response?.data?.code === 1) {
            ...
            toast.success('Experience level submitted successfully!', { position: 'bottom-right' });
          } else {
  ...
  const formikForm = useFormik({
    ...
    onSubmit: (values) => {
      ...
      dispatch(setProgress(10));
      toast.success('Experience added successfully!', { position: 'bottom-right' });
  ...
  return (
    <div className="">
      <h2 className="text-center font-semibold text-lg md:text-xl xl:text-[28px] 2xl:leading-[38px]">
        You’re almost there! Add<br /> your <span className="text-red">experience</span>
      </h2>
      <form onSubmit={formik.handleSubmit} className="block mt-8 md:mt-10 xl:mt-14 2xl:mt-16">
        ...
        <div className="my-4 flex flex-col sm:flex-row gap-4">
          <label 
          htmlFor="fresher"              
          className={`form-group !flex flex-1 !mb-0 gap-4 rounded-lg px-5 py-4 border cursor-pointer shadow-sm items-center ${2 === formik.values.is_fresher ? 'border-red bg-[#FDF1F3]' : 'border-[#C8C9CB1A]'}`}
          >
            <input
              type="radio"
              id="fresher"
          ...
          <label  
          htmlFor="experienced"             
          className={`form-group !flex flex-1 !mb-0 gap-4 rounded-lg px-5 py-4 border cursor-pointer shadow-sm items-center ${1 === formik.values.is_fresher ? 'border-red bg-[#FDF1F3]' : 'border-[#C8C9CB1A]'}`}
          >
            <input
              type="radio"
              id="experienced"
        ...
        <div className="flex w-full items-end">
          ...
          <div className="flex gap-4 items-end w-full justify-end">
            <span
              ...
              className="max-w-[130px] cursor-pointer hover:border-gray-400 flex items-center btn-border h-[56px]"
            >
              Skip
            </span>
            <button
