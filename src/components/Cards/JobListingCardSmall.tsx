...
export default function JobListingCardSmall({detail}:{detail:CompanyJob}) {
  ...
  useEffect(() => {
    setIsFavorited(detail?.saveJob_status=='2')
  }, [detail])
    ...
    <div className='job-card w-full border shadow-default border-lightGrey rounded-2xl bg-white p-4 md:p-5 xl:p-6'>
      ...
      <div className="flex">
        <div className="flex">
          ...
          <span className='ml-2 text-xs 2xl:text-sm text-[#545581] whitespace-nowrap'>{showExperience(detail?.freshers_can_apply, detail?.min_exp, detail?.max_exp)}</span>
        </div>
        <span className='ml-5 text-xs 2xl:text-sm text-[#545581] whitespace-nowrap'>{showSalary(detail?.salary_range_unit, detail?.min_salary, detail?.max_salary, detail?.is_industry_standard)}</span>
      </div>
      <div className="flex flex-wrap gap-4 min-h-16 justify-between">
        <ul className='flex flex-wrap gap-2 mt-3'>
          <li className='label !text-xs 2xl:!text-sm'>Problem Solving</li>
          <li className='label !text-xs 2xl:!text-sm green'>time management</li>
          <li className='label !text-xs 2xl:!text-sm'>adaptability</li>
        </ul>
