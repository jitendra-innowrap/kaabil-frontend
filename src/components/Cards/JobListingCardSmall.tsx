...
export default function JobListingCardSmall({detail}:{detail:CompanyJob}) {
  ...
  return (
    <div className='job-card small w-full border shadow-sm border-lightGrey rounded-2xl bg-white p-4 3xl:p-6'>
      ...
      <div className="flex justify-between gap-y-2">
        <div className="flex">
          ...
          <span className='ml-2 text-[10px] 2xl:text-sm text-[#545581] whitespace-nowrap'>{showExperience(detail?.min_exp, detail?.max_exp)}</span>
        </div>
        <div className='ml-5 text-[10px] 2xl:text-sm text-[#545581] text-end'>{showSalary(detail?.is_industry_standard, detail?.salary_range_unit, detail?.min_salary, detail?.max_salary)}</div>
      </div>
      <div className="flex flex-wrap gap-4 min-h-16 justify-between">
        <ul className='flex flex-wrap gap-2 mt-3'>
          {
            detail?.jobs_skills?.slice(0, 4)?.map((skill)=>(
              <li className='label'>{skill?.name}</li>
            ))
          }{
            detail?.jobs_skills?.length > 4 && (
              <li className='label'>+{(detail?.jobs_skills?.length - 4).toString()}</li>
            )
          }
        </ul>
